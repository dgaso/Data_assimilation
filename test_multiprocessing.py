...
def gather_all_pixels():
    ...
    return relevant_pixels, index,index1,index2,index3,index4, index5,index6,index7
...
def optimizer_with_mp():
    ...
    """
    years = [2018, ]
    pixels_for_DA, index,index1,index2, index3, index4, index5,index6,index7 = gather_all_pixels()
...
#            print(f" - {parname}: {value}")
    ...
    with rasterio.open('Ashby_yield.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(result_yield,0))
    with rasterio.open('Ashby_WUE.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(result_p1,0))
    with rasterio.open('Ashby_RDMAX.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(result_p2,0))
    with rasterio.open('Ashby_FNTR.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(result_p3,0))
    with rasterio.open('Ashby_initLAI.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(result_p4,0))
    with rasterio.open('Ashby_LAImax.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(results_max_lai,0))
    with rasterio.open('Ashby_defveg.tif', 'w', **meta) as outds:
        outds.write(np.expand_dims(results_def_veg,0))
    with rasterio.open('Ashby_defrep.tif', 'w', **meta) as outds:
